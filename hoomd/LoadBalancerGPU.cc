...
void LoadBalancerGPU::countParticlesOffRank(std::map<unsigned int, unsigned int>& cnts)
    ...
    if (m_pdata->getN() == 0)
        ...
        m_tuner->begin();
        kernel::gpu_load_balance_mark_rank(m_exec_conf->getStream(), d_comm_flag.data,
                                           d_pos.data,
                                           d_cart_ranks.data,
    ...
    vector<unsigned int> off_rank;
        ...
        const unsigned int n_off_rank
            = kernel::gpu_load_balance_select_off_rank(m_exec_conf->getStream(), d_off_ranks.data,
                                                       d_comm_flag.data,
                                                       m_pdata->getN(),
                                                       m_exec_conf->getRank());
